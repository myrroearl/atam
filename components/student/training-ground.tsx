    ...
    <div className="max-w-2xl mx-auto space-y-6 px-4 sm:px-6">
          ...
          <div className="flex items-center space-x-2 w-full sm:w-auto">
            ...
            <div className="flex items-center space-x-2 flex-1 sm:flex-initial">
              ...
              <select
                ...
                className="px-3 py-2 border border-input bg-background rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-ring min-w-[80px]"
              >
      ...
      {!loading && !error && resources.length > 0 && (
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          <span>
            {showBookmarksOnly && `${bookmarkedResources.size} bookmarked resources`}
            {!showBookmarksOnly && `${filteredResources.length} resources available`}
          </span>
        ...
        <div className="space-y-4">
          {filteredResources.length === 0 ? (
            ...
            paginatedResources.map((resource) => (
              <Card key={resource.id} className="glass-card shadow-card-lg hover:shadow-card-xl transition-all duration-300 group overflow-hidden">
                    ...
                    <div className="flex items-start space-x-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center flex-wrap gap-2 mb-1">
                          <span className="font-semibold text-foreground truncate">{resource.author}</span>
                          {resource.isLowPerformance && (
                        ...
                        <div className="flex items-center flex-wrap gap-2 text-sm text-muted-foreground">
                          <div className="flex items-center space-x-1">
                            ...
                            <span className="font-medium truncate">{resource.source}</span>
                          </div>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <h3 className="text-xl font-bold leading-tight text-foreground group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors duration-200 break-words">
                        {resource.title}
                      </h3>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-2 border-t border-muted/50">
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-muted-foreground">
                        <div className="flex items-center space-x-1">
                          ...
                          <span className="whitespace-nowrap">Relevance: {Math.round(resource.relevanceScore)}</span>
                        </div>
                        <div className="flex items-center space-x-1">
                          <TrendingUp className="w-3 h-3 flex-shrink-0" />
                          <span className="whitespace-nowrap">{resource.likes - resource.dislikes} net likes</span>
                        </div>
                        ...
                        {resourceStats[resource.id] && (
                          <div className="flex items-center space-x-1">
                            ...
                            <span className="text-muted-foreground text-xs whitespace-nowrap">
                              {resourceStats[resource.id].totalOpens} total opens
                            </span>
                          </div>
                        )}
                      </div>
                  ...
                  <div className="p-4 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/20 rounded-xl border border-green-200/50 dark:border-green-800/50 group-hover:from-green-100 group-hover:to-emerald-100 dark:group-hover:from-green-950/30 dark:group-hover:to-emerald-950/30 transition-all duration-300">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <Button 
                        size="sm" 
                        className="bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white shadow-lg hover:shadow-xl transition-all duration-300 group-hover:scale-105 flex-shrink-0"
